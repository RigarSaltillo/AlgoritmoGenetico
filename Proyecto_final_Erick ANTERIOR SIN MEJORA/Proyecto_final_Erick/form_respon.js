...
function obtenerNombres() {
    var cantidadNombres = parseInt(document.getElementById("cantidadNombres").value);
    var nombres = [];

    for (var i = 1; i <= cantidadNombres; i++) {
        var nombre = document.getElementById("nombre" + i).value;
        nombre=nombre.toUpperCase()
        nombres.push(nombre.trim());
    }

    // Ahora, puedes hacer lo que necesites con el arreglo "nombres"
    console.log(nombres);
    return nombres;
}
...
let m =document.getElementById('cantidadNombres');

let l1,l2,l3
let ma=[]
let media = document.getElementById("formulario").addEventListener("submit", function (event) {
    event.preventDefault(); // Prevenir el envío del formulario por defecto
    ma=obtenerNombres();
    let n =document.getElementById('secuencias');
